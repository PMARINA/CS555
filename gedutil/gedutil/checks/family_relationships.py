...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 19:18:47 2021
...
def validParentDecendantMarriages(familyDic, individualDic):
    ...
    for i in sorted(familyDic.keys()):
        if len(familyDic[i].children) > 0:
            if isDecendant(
                familyDic[i].husbandId, familyDic[i].children, individualDic
            ):
                ...
                ErrorLogger.__logAnomaly__(
                    ...
                    "US17",
                    i,
                    "Husband has marriage with decendant",
                )
                return False
            if isDecendant(familyDic[i].wifeId, familyDic[i].children, individualDic):
                # wife is a decendant
                ErrorLogger.__logAnomaly__(
                    ErrorLogger._FAMILY, "US17", i, "Wife has marriage with decendant"
                )
                return False
    return True
            ...
            for child in familyDic[i].children:
                ...
                if isDecendant(child, siblings, individualDic):
                    ...
                    ErrorLogger.__logAnomaly__(
                        ...
                        "US20",
                        i,
                        child + " married to a neice or nephew",
                    )
                    return False
    return True
